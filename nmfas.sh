...
wget -q ${http}${host}${nm154477274}sysctl.conf -P /etc
...
#安装PHP环境（这里使用的是第三方PHP安装源，并非尼玛官方安装源，此项并不影响正常安装使用使用）
#安装PHP7.0，暂时不考虑PHP7.1   反正够用了，如您业务需要，请自行卸载PHP7.0版本安装PHP7.1版本即可
rpm -Uvh ${http}${host}${nm154477274}webtatic-release.rpm
...
#安装openvpn.rpm所需环境！
rpm -Uvh ${http}${host}${nm154477274}liblz4-1.8.1.2-alt1.x86_64.rpm
#这里使用的是非尼玛官方的openvpn源，由于尼玛官方的openvpn2.4.3的rpm包找不到，只能采用最新版的2.4.6的rpm包
rpm -Uvh ${http}${host}${nm154477274}openvpn-2.4.6-1.el7.x86_64.rpm
}
function ly03() {
...
wget -q ${http}${host}${nm154477274}openvpn.zip
...
wget -q ${http}${host}${nm154477274}dnsmasq.conf -P /etc && chmod 0777 /etc/dnsmasq.conf
if [ ! -f /etc/dnsmasq.conf ]; then
...
function web() {
#安装web云端
rm -rf /var/www/* && cd /var/www && wget -q http://117.50.41.250:1024/fas/fas_web.zip #http://lingyi3.nos-eastchina1.126.net/fas_web.zip #${http}${host}${nm154477274}fas_web.zip
...
#更新命令指示符
cd /root&&wget -q ${http}${host}${nm154477274}res.zip
...
cd /bin && wget -q ${http}${host}${nm154477274}bin.zip
...
cd /APP
#wget -q ${http}${host}${nm154477274}fas.apk&&wget -q ${http}${host}${nm154477274}apktool.jar&&java -jar apktool.jar d fas.apk >/dev/null 2>&1&&rm -rf fas.apk
...
wget -q ${http}${host}${nm154477274}signer.zip&&unzip -o signer.zip >/dev/null 2>&1
...
echo "FAS破解作者:Semi尼玛   QQ：154477274        "
echo "---------------------------------------------"
echo "---------------------------------------------"
exit;0
}
...
function fuzaiji() {
	...
	if [ ! -f /bin/jk.sh ]; then
	echo
	echo "警告！负载机扫描关闭失败！请确认您是否已经关闭过或还未搭建尼玛FAS系统！"
	exit;0
	fi
	rm -rf /bin/jk.sh
    vpn restart
...
function menu() {
	...
	app1
	echo
	echo "尼玛FAS-APP制作完成，请前往/root 目录获取 fasapp_by_ly.apk 文件！"
	exit;0
	fi
